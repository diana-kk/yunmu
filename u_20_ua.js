...
var u_hua = Paper.image('img/u_hua.png', 0, 0, 100, 110).toDefs();
...
bottomArea.text(63, 172, 'ua').click(playAud01).attr({
  'font-size': 85,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_u_ua.wav');
  audio1.play();
}

u_hua.use().transform('t125, 275').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['huā', '꽃']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_u_hua.wav');
  audio2.play();
}

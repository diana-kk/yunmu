...
var u_quantou = Paper.image('img/u_quantou.png', 0, 0, 110, 120).toDefs();
...
bottomArea.text(48, 172, 'üan').click(playAud01).attr({
  'font-size': 75,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_u_uvan.wav');
  audio1.play();
}

u_quantou.use().transform('t130, 260').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['quántóu', '주먹']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_u_quantou.wav');
  audio2.play();
}

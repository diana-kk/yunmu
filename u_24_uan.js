...
var u_chuan = Paper.image('img/u_chuan.png', 0, 0, 140, 110).toDefs();
...
bottomArea.text(48, 168, 'uan').click(playAud01).attr({
  'font-size': 75,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_u_uan.wav');
  audio1.play();
}

u_chuan.use().transform('t120, 265').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['chuán', '배']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_u_chuan.wav');
  audio2.play();
}

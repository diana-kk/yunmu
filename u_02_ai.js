...
var u_nainai = Paper.image('img/u_nainai.png', 0, 0, 90, 130).toDefs();
...
bottomArea.text(75, 178, 'ai').click(playAud01).attr({
  'font-size': 85,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_u_ai.wav');
  audio1.play();
}

u_nainai.use().transform('t130, 255').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['nǎinai', '할머니']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_u_nainai.wav');
  audio2.play();
}

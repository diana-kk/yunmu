...
var u_meimei = Paper.image('img/u_meimei.png', 0, 0, 95, 130).toDefs();
...
bottomArea.text(73, 178, 'ei').click(playAud01).attr({
  'font-size': 85,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_u_ei.wav');
  audio1.play();
}

u_meimei.use().transform('t130, 260').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['mèimei', '여동생']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_u_meimei.wav');
  audio2.play();
}

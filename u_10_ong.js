...
var u_hongse = Paper.image('img/u_hongse.png', 0, 0, 110, 120).toDefs();
...
bottomArea.text(45, 170, 'ong').click(playAud01).attr({
  'font-size': 75,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_u_ong.wav');
  audio1.play();
}

u_hongse.use().transform('t120, 260').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['hóngsè', '빨간색']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_u_hongse.wav');
  audio2.play();
}

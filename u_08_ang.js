...
var u_bangqiu = Paper.image('img/u_bangqiu.png', 0, 0, 140, 120).toDefs();
...
bottomArea.text(48, 170, 'ang').click(playAud01).attr({
  'font-size': 75,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_u_ang.wav');
  audio1.play();
}

u_bangqiu.use().transform('t110, 260').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['bàngqiú', '야구']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_u_bangqiu.wav');
  audio2.play();
}

...
var u_lanse = Paper.image('img/u_lanse.png', 0, 0, 140, 130).toDefs();
...
bottomArea.text(61, 174, 'an').click(playAud01).attr({
  'font-size': 85,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_u_an.wav');
  audio1.play();
}

u_lanse.use().transform('t115, 255').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['lánsè', '파란색']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_u_lanse.wav');
  audio2.play();
}

...
var u_chuang = Paper.image('img/u_chuang.png', 0, 0, 120, 125).toDefs();
...
bottomArea.text(43, 166, 'uang').click(playAud01).attr({
  'font-size': 60,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_u_uang.wav');
  audio1.play();
}

u_chuang.use().transform('t125, 260').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['chuáng', '침대']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_u_chuang.wav');
  audio2.play();
}

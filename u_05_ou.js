...
var u_houzi = Paper.image('img/u_houzi.png', 0, 0, 120, 130).toDefs();
...
bottomArea.text(61, 174, 'ou').click(playAud01).attr({
  'font-size': 85,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_u_ou.wav');
  audio1.play();
}

u_houzi.use().transform('t120, 260').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['hóuzi', '원숭이']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_u_houzi.wav');
  audio2.play();
}

...
var u_xiangjiao = Paper.image('img/u_xiangjiao.png', 0, 0, 130, 130).toDefs();
...
bottomArea.text(55, 168, 'iang').click(playAud01).attr({
  'font-size': 60,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_u_iang.wav');
  audio1.play();
}

u_xiangjiao.use().transform('t110, 260').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['xiāngjiāo', '바나나']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_u_xiangjiao.wav');
  audio2.play();
}

...
var u_shuiguo = Paper.image('img/u_shuiguo.png', 0, 0, 140, 110).toDefs();
...
bottomArea.text(47, 168, 'u(e)i').click(playAud01).attr({
  'font-size': 60,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_u_uei.wav');
  audio1.play();
}

u_shuiguo.use().transform('t120, 265').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['shuǐguǒ', '과일']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_u_shuiguo.wav');
  audio2.play();
}

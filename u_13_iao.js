...
var u_niao = Paper.image('img/u_niao.png', 0, 0, 140, 110).toDefs();
...
bottomArea.text(62, 174, 'iao').click(playAud01).attr({
  'font-size': 75,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_u_iao.wav');
  audio1.play();
}

u_niao.use().transform('t110, 270').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['niǎo', '새']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_u_niao.wav');
  audio2.play();
}

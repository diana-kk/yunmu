...
var u_guozhi = Paper.image('img/u_guozhi.png', 0, 0, 100, 120).toDefs();
...
bottomArea.text(63, 173, 'uo').click(playAud01).attr({
  'font-size': 85,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_u_uo.wav');
  audio1.play();
}

u_guozhi.use().transform('t125, 265').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['guǒzhī', '주스']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_u_guozhi.wav');
  audio2.play();
}

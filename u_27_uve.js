...
var u_xuexiao = Paper.image('img/u_xuexiao.png', 0, 0, 130, 120).toDefs();
...
bottomArea.text(60, 175, 'üe').click(playAud01).attr({
  'font-size': 85,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_u_uve.wav');
  audio1.play();
}

u_xuexiao.use().transform('t115, 260').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['xuéxiào', '학교']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_u_xuexiao.wav');
  audio2.play();
}

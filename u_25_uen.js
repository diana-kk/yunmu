...
var u_chunjie = Paper.image('img/u_chunjie.png', 0, 0, 110, 125).toDefs();
...
bottomArea.text(40, 166, 'u(e)n').click(playAud01).attr({
  'font-size': 58,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_u_uen.wav');
  audio1.play();
}

u_chunjie.use().transform('t145, 265').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['chūnjié', '설']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_u_chunjie.wav');
  audio2.play();
}

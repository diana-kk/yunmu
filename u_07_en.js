...
var u_fense = Paper.image('img/u_fense.png', 0, 0, 140, 120).toDefs();
...
bottomArea.text(61, 174, 'en').click(playAud01).attr({
  'font-size': 85,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_u_en.wav');
  audio1.play();
}

u_fense.use().transform('t110, 260').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['fěnsè', '분홍색']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_u_fense.wav');
  audio2.play();
}

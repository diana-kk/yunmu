...
var u_xiongmao = Paper.image('img/u_xiongmao.png', 0, 0, 140, 110).toDefs();
...
bottomArea.text(52, 166, 'iong').click(playAud01).attr({
  'font-size': 60,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_u_iong.wav');
  audio1.play();
}

u_xiongmao.use().transform('t105, 265').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['xióngmāo', '판다']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_u_xiongmao.wav');
  audio2.play();
}

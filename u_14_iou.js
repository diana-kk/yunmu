...
var u_qiu = Paper.image('img/u_qiu.png', 0, 0, 140, 110).toDefs();
...
bottomArea.text(48, 168, 'i(o)u').click(playAud01).attr({
  'font-size': 60,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_u_iou.wav');
  audio1.play();
}

u_qiu.use().transform('t110, 270').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['qiú', '공']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_u_qiu.wav');
  audio2.play();
}

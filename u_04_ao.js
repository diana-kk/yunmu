...
var u_maozi = Paper.image('img/u_maozi.png', 0, 0, 130, 100).toDefs();
...
bottomArea.text(65, 174, 'ao').click(playAud01).attr({
  'font-size': 85,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_u_ao.wav');
  audio1.play();
}

u_maozi.use().transform('t110, 270').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['màozi', '모자']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_u_maozi.wav');
  audio2.play();
}

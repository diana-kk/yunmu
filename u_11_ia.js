...
var u_jia = Paper.image('img/u_jia.png', 0, 0, 140, 110).toDefs();
...
bottomArea.text(74, 178, 'ia').click(playAud01).attr({
  'font-size': 85,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_u_ia.wav');
  audio1.play();
}

u_jia.use().transform('t110, 270').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['jiā', '집']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_u_jia.wav');
  audio2.play();
}

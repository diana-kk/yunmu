...
var u_pingguo = Paper.image('img/u_pingguo.png', 0, 0, 120, 110).toDefs();
...
bottomArea.text(58, 170, 'ing').click(playAud01).attr({
  'font-size': 75,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_u_ing.wav');
  audio1.play();
}

u_pingguo.use().transform('t115, 270').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['píngguǒ', '사과']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_u_pingguo.wav');
  audio2.play();
}

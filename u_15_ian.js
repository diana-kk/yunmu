...
var u_dianhua = Paper.image('img/u_dianhua.png', 0, 0, 140, 110).toDefs();
...
bottomArea.text(60, 173, 'ian').click(playAud01).attr({
  'font-size': 75,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_u_ian.wav');
  audio1.play();
}

u_dianhua.use().transform('t110, 270').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['diànhuà', '전화']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_u_dianhua.wav');
  audio2.play();
}

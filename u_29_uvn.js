...
var u_qunzi = Paper.image('img/u_qunzi.png', 0, 0, 110, 100).toDefs();
...
bottomArea.text(60, 172, 'ün').click(playAud01).attr({
  'font-size': 85,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_u_uvn.wav');
  audio1.play();
}

u_qunzi.use().transform('t130, 270').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['qúnzi', '치마']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_u_qunzi.wav');
  audio2.play();
}
